...
specificity <- read.csv(file.path(article_root, "data", "cultural_specificity_notes.csv"), stringsAsFactors = FALSE)

summary <- merge(sources, patterns, by = "item", all = TRUE)
summary <- merge(summary, specificity, by = "item", all = TRUE)

write.csv(summary, file.path(tables_dir, "comparative_myth_source_pattern_summary.csv"), row.names = FALSE)
print(summary)
