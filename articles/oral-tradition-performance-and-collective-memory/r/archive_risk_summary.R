...
write.csv(risk_summary, file.path(tables_dir, "archive_risk_summary.csv"), row.names = FALSE)

png(file.path(figures_dir, "archive_risk_summary.png"), width = 1000, height = 700)
barplot(
  risk_summary$severity_score,
  names.arg = risk_summary$item,
  las = 2,
  ylab = "Archive risk severity",
  main = "Archive Risk Summary"
)
grid()
dev.off()
