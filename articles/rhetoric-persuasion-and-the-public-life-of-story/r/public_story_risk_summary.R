...
risks <- read.csv(file.path(article_root, "data", "public_story_risks.csv"), stringsAsFactors = FALSE)
...
write.csv(risk_summary, file.path(tables_dir, "public_story_risk_summary.csv"), row.names = FALSE)

png(file.path(figures_dir, "public_story_risk_summary.png"), width = 1000, height = 700)
barplot(
  risk_summary$severity_score,
  names.arg = risk_summary$item,
  las = 2,
  ylab = "Risk severity",
  main = "Public Story Risk Summary"
)
grid()
dev.off()
