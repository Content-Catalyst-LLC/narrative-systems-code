...
class CatalystCanvasAdvancedTests(unittest.TestCase):
    def setUp(self) -> None:
        self.config = CanvasConfig(
            article_title="Sacred History and Revelatory Narrative",
            article_slug="sacred-history-and-revelatory-narrative",
    ...
    def test_scores_are_bounded(self) -> None:
        record = CanvasRecord(
            item="Claim", claim_context="Context", article_title="Sacred History and Revelatory Narrative", article_slug="sacred-history-and-revelatory-narrative",
            ...
            owner="editorial", status="active",
        )
        validate_record(record, self.config)
        for value in [domain_strength(record, self.config), risk_score(record, self.config), interpretation_readiness(record, self.config), governance_priority_score(record, self.config)]:
