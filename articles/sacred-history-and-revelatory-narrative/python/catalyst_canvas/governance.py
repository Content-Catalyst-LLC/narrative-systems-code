...
def governance_note(record: CanvasRecord, config: CanvasConfig) -> str:
    ...
    if domain_strength(record, config) < 0.55:
        notes.append("Domain signal is thin; verify support before reuse.")
    if record.status == "revise":
        notes.append("Record status is revise; prioritize correction before publication or reuse.")
    if record.notes:
...
def build_canvas_card(record: CanvasRecord, config: CanvasConfig) -> dict[str, Any]:
    return {
        ...
        "signals": {"metrics": record.metrics, "risk_signals": record.risk_signals, "readiness_signals": record.readiness_signals},
    }
