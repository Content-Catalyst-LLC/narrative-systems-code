...
risks <- read.csv(file.path(article_root, "data", "ethical_story_risks.csv"), stringsAsFactors = FALSE)
...
write.csv(risk_summary, file.path(tables_dir, "ethical_risk_summary.csv"), row.names = FALSE)

png(file.path(figures_dir, "ethical_risk_summary.png"), width = 1000, height = 700)
barplot(
  risk_summary$severity_score,
  names.arg = risk_summary$item,
  las = 2,
  ylab = "Ethical risk score",
  main = "Ethical Story Risk Summary"
)
grid()
dev.off()
