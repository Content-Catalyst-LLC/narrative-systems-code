...
risks <- read.csv(file.path(article_root, "data", "conflict_risks.csv"), stringsAsFactors = FALSE)
...
write.csv(risk_summary, file.path(tables_dir, "conflict_risk_summary.csv"), row.names = FALSE)

png(file.path(figures_dir, "conflict_risk_summary.png"), width = 1000, height = 700)
barplot(
  risk_summary$severity_score,
  names.arg = risk_summary$item,
  las = 2,
  ylab = "Conflict risk severity",
  main = "Conflict Risk Summary"
)
grid()
dev.off()
