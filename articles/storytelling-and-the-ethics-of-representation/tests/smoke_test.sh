...
python3 "${ARTICLE_ROOT}/python/run_representation_ethics_governance_audit.py" --article-root "${ARTICLE_ROOT}"
python3 "${ARTICLE_ROOT}/python/run_catalyst_canvas_audit.py" --article-root "${ARTICLE_ROOT}" --strict

if command -v Rscript >/dev/null 2>&1; then
  Rscript "${ARTICLE_ROOT}/r/run_all_representation_ethics_governance_workflows.R"
else
