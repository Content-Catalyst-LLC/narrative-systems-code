...
def validate_record(record: CanvasRecord, config: CanvasConfig, strict: bool = False) -> None:
    ...
    if strict:
        checks = [
            ("metric fields", set(config.metric_weights), set(record.metrics)),
            ("risk fields", set(config.risk_weights), set(record.risk_signals)),
            ("readiness fields", set(config.readiness_weights), set(record.readiness_signals)),
        ]
