    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        for row in csv.DictReader(handle):
            record = CanvasRecord(
                ...
                claim_context=row.get("claim_context", row.get("representation_context", "")).strip(),
...
def build_rows(records: list[CanvasRecord], config: CanvasConfig) -> list[dict[str, Any]]:
    ...
    return sorted(rows, key=lambda row: (priority_order.get(str(row["review_priority"]), 0), float(row["governance_priority_score"])), reverse=True)


def run(article_root: Path, input_path: Path | None = None, config_path: Path | None = None, output_dir: Path | None = None, strict: bool = False) -> None:
...
def main() -> None:
    ...
    run(args.article_root, args.input, args.config, args.output_dir, args.strict)
